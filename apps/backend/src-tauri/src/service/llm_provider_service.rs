...
use crate::service::llm_gateway_service::adapters::OpenAIAdapter;
use crate::service::llm_gateway_service::LLMProviderAdapter;
...
pub async fn fetch_models_from_api(provider_id: i64) -> Result<Vec<LlmModel>, String> {
    ...
    let base_url = provider.base_url.clone().unwrap_or_else(|| {
        crate::service::llm_gateway_service::get_default_base_url(&provider.provider_code)
    });
