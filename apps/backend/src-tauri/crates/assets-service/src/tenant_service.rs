...
use assets_database::models::{SysTenant, TenantInfo};
...
pub struct TenantResponse {
    ...
    #[serde(
        serialize_with = "assets_database::models::opt_i64_to_string",
        deserialize_with = "assets_database::models::opt_i64_from_string"
    )]
...
pub async fn insert_tenant(
    ...
    if is_leaf {
        ...
        // 初始化租户 schema 表结构
        info!("正在初始化租户 '{}' 表结构...", sn);
        assets_database::postgres::init_tenant_tables(&pool, sn)
            .await
            .map_err(|e| {
        ...
        assets_database::postgres::init_tenant_default_data(&pool, sn)
            .await
            .map_err(|e| {
...
pub async fn switch_tenant(user_id: i64, tenant_id: i64) -> Result<TenantInfo, String> {
    ...
    let schema_name = assets_database::postgres::get_schema_by_tenant_id(&pool, tenant_id).await?;

    // 4. 更新 USER_TENANT_CACHE
    let cache = assets_database::postgres::get_user_tenant_cache();
...
pub async fn assign_user_tenants(
    ...
    for tid in tenant_ids {
        ...
        .bind(assets_utils::snowflake::next_id() as i64)
        .bind(user_id)
        .bind(tid)
