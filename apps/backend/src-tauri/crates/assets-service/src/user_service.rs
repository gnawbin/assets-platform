use assets_database::models::{SysTenant, SysUser, TenantInfo};
use assets_database::{get_read_pool, get_write_pool};
use assets_utils::password_secret::{hash_password, verify_password};
...
pub struct LoginResponse {
    ...
    #[serde(serialize_with = "assets_database::models::opt_i64_to_string")]
    ...
    if let Some(tenant_id) = user.tenant_id {
        ...
        if let Some(tenant) = available_tenants.iter().find(|t| t.id == tenant_id) {
            if let Some(ref sn) = tenant.schema_name {
                assets_database::set_global_schema(sn.clone());
            }
        }
        info!(
...
pub async fn insert_user(
    ...
    if user.is_super_admin {
        ...
        if !tenant_ids.is_empty() {
            crate::tenant_service::assign_user_tenants(
                user.id,
                &tenant_ids,
