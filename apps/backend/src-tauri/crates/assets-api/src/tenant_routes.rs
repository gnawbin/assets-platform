...
use assets_database::models::TenantInfo;
use assets_service;
...
pub async fn get_tenants() -> Result<Json<ApiResponse<Vec<TenantResponse>>>, ApiError> {
    match assets_service::tenant_service::get_tenants().await {
...
pub async fn insert_tenant(
    ...
    match assets_service::tenant_service::insert_tenant(
        &req.tenant_name,
        parent_id,
        req.is_leaf,
...
pub async fn update_tenant(
    ...
    match assets_service::tenant_service::update_tenant(id, &req.tenant_name, req.enable).await {
        Ok(tenant) => Ok(Json(ApiResponse::success(tenant))),
        Err(e) => Err(ApiError::internal_error(e)),
    }
}
...
pub async fn delete_tenant(Path(id): Path<String>) -> Result<Json<ApiResponse<()>>, ApiError> {
    ...
    match assets_service::tenant_service::delete_tenant(id).await {
...
pub async fn switch_tenant(
    ...
    match assets_service::tenant_service::switch_tenant(ctx.user_id, tenant_id).await {
...
pub async fn assign_tenants(
    ...
    match assets_service::tenant_service::assign_user_tenants(user_id, &tenant_ids, ctx.user_id).await {
        Ok(_) => Ok(Json(ApiResponse::success(()))),
        Err(e) => Err(ApiError::internal_error(e)),
    }
}
...
pub async fn get_user_tenants(
    ...
    match assets_service::tenant_service::get_user_tenants(user_id).await {
        Ok(tenants) => Ok(Json(ApiResponse::success(tenants))),
        Err(e) => Err(ApiError::internal_error(e)),
    }
}
