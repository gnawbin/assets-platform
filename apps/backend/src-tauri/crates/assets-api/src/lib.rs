...
use assets_engine::skill_registry::SkillRegistry;
...
pub async fn start_http_server(
    pool: sqlx::PgPool,
    llm_router: Option<std::sync::Arc<assets_service::llm_gateway_service::LLMRouter>>,
) -> anyhow::Result<()> {
    ...
    let upload_router = {
        let s3_config = assets_storage::s3::S3Config::from_env().unwrap_or_default();
        let s3_client = assets_storage::s3::S3Client::new(s3_config.clone())
            .await
            .map_err(|e| anyhow::anyhow!("S3 客户端初始化失败: {:?}", e))?;
        let upload_mgr =
            assets_storage::upload::UploadManager::new(pool.clone(), s3_client, s3_config);
