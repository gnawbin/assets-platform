...
use assets_database::models::AssetCategory;
use assets_service;
...
pub async fn get_categories() -> Result<Json<ApiResponse<Vec<AssetCategory>>>, ApiError> {
    match assets_service::assets_categories_service::get_categories().await {
...
pub async fn get_categories_parents() -> Result<Json<ApiResponse<Vec<AssetCategory>>>, ApiError> {
    match assets_service::assets_categories_service::get_super_categories().await {
        Ok(categories) => Ok(Json(ApiResponse::success(categories))),
        Err(e) => Err(ApiError::internal_error(e)),
    }
}
...
pub async fn insert_category(
    ...
    match assets_service::assets_categories_service::insert_category(&category).await {
...
pub async fn update_category(
    ...
    match assets_service::assets_categories_service::update_category(&category).await {
        Ok(cat) => Ok(Json(ApiResponse::success(cat))),
        Err(e) => Err(ApiError::internal_error(e)),
    }
}
...
pub async fn delete_category(Path(id): Path<String>) -> Result<Json<ApiResponse<()>>, ApiError> {
    ...
    match assets_service::assets_categories_service::delete_category(id).await {
