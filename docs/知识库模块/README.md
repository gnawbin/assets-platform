...
| 菜单项 | 路径 | order_num | 状态 | 说明 |
|--------|------|-----------|------|------|
| 📖 **知识库管理** | `/knowledge` | 1 | ✅ 已有 | 知识树 + Markdown编辑器 + 文件上传 |
| 💬 **智能问答** | `/chat` | 2 | ❌ 待建 | 多轮对话 + RAG检索 + 溯源引用 |
| ⚡ **Skill 管理** | `/skills` | 3 | ✅ 已有 | 从原 AI 工作流移入 |
| 🧠 **LLM厂商管理** | `/settings/llm` | 4 | ❌ 待建 | 厂商密钥/模型/权重管理 |
| 👤 **模型偏好** | `/settings/llm/preference` | 5 | ❌ 待建 | 用户默认模型/温度/Token配置 |
...
| 多模态问答设计 | [智能问答多模态（文件与视频上传）设计方案.md](./智能问答多模态（文件与视频上传）设计方案.md) | 聊天页文件/视频上传、视觉 LLM 理解、doc-parser 解析接入 |
