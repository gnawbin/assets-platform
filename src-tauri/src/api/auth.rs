...
async fn resolve_user_schema(user_id: i64) -> String {
    ...
    let user_cache = crate::database::postgres::get_user_tenant_cache();
    if let Some(entry) = user_cache.get(&user_id) {
        let tenant_id = *entry;
        // 查 SCHEMA_CACHE 获取 schema 名称
        let schema_cache = crate::database::postgres::get_schema_cache();
    ...
    // 2. 缓存未命中，从数据库兜底查询
    if let Ok(pool) = crate::database::get_read_pool() {
        // 查询用户的 tenant_id
        ...
        if let Some(tid) = tenant_id {
            ...
            if let Some(sn) = schema {
                ...
                crate::database::postgres::get_schema_cache().insert(tid, sn.clone());
                return sn;
            }
        }
    }
...
pub async fn auth_middleware(mut req: Request, next: Next) -> Result<Response, ApiError> {
    ...
    let response = crate::database::with_schema(schema_name, async { next.run(req).await }).await;
